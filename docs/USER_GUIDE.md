...
Open **Income → Projection** to estimate gross NPC salvage value over **30, 90, or 365 days** at your current farming pace. The default horizon is **365 days**. History retains its existing calculations and controls. The History/Projection selection, projection horizon, projection-chart visibility, and **Allow previous-rank approximations** toggle are **Saved automatically**, independently of staged FC or global settings. The approximation toggle is enabled by default, including when upgrading an existing configuration.
...
The estimate uses valid recorded returns from the **last 90 days**, including returns worth **0 gil**. The route's unordered sector set must match; the ordered current/pinned route determines duration. At least **10 matching returns** are required in the chosen sample. The planner chooses the first sufficient sample in this order:

1. This submarine's history matching its exact current surveillance, retrieval, and favor.
2. History matching those current stats pooled across all visible FCs.
3. This submarine's compatible history from the immediately previous rank, when approximations are enabled.
4. Compatible previous-rank history pooled across all visible FCs.

For a previous-rank approximation, the planner resolves today's parts at **current rank minus one**. Only returns recorded at that rank, with exactly those surveillance/retrieval/favor values and the same sector set, qualify. For example, a rank-144 submarine with stats **203/254/223** can use rank-143 history with **203/252/222** if that is what its current parts resolve to at rank 143. This matches recorded stats; it does not reconstruct or assert identical historical parts. The planner does not search older ranks or combine insufficient exact and previous-rank samples to reach ten. Disable the toggle to require exact current stats only.

Pooling includes this submarine's returns exactly once. Historical returns remain usable even if their submarine is now leveling or paused. Hidden FCs never contribute. An FC's Income shortcut narrows displayed totals while retaining the visible-FC comparison pool. Exact matches always take priority; an approximation automatically switches to exact matching once enough exact history is available.

Average gil per voyage is the sample's gross salvage value divided by all matching returns. Estimated gil per day divides that average by the **current** full cycle length in days; the period total multiplies by 30, 90, or 365. A previous-rank approximation reuses its historical average without adjusting for changed stats, and still uses today's speed, ordered route, and collection delay. Fractional cycles are intentional: this is a steady estimate, not an exact collection schedule. For example, four submarines averaging 100,000 gil each per 48-hour cycle produce an estimated **73 million gil over 365 days**.
