...
public sealed partial class PlannerWindow
{
    ...
    private void DrawIncomeProjection(EtaPlannerSnapshot currentSnapshot)
    {
                ...
                ImGuiTableFlags.SizingStretchSame))
        {
            ...
            PlannerUi.MetricCard(this.typography, "projection-total", FontAwesomeIcon.Coins,
                ...
                totals.IsPartial ? "Estimated gil · partial" : "Estimated gil", totals.IsPartial ? PlannerUi.Amber : PlannerUi.Green);
            ImGui.TableNextColumn();
    ...
    private void DrawIncomeProjectionFleet(IncomeFcProjection fc, IncomeProjectionHorizon horizon, CompactFcHeaderLayout layout)
    {
        ...
        ImGui.PopStyleColor();
        ImGui.PopStyleVar();
        var color = fc.Totals.IsPartial ? PlannerUi.Amber : ImGui.GetStyle().Colors[(int)ImGuiCol.Text];
        ...
        foreach (var sub in fc.Submarines.Where(sub => !sub.IsAvailable))
            PlannerUi.WrappedText($"{sub.Name}: {sub.UnavailableReason}" +
                (sub.Sample.ReturnCount < IncomeProjectionCalculator.MinimumReturns
                    ? $" · {sub.Sample.ReturnCount} / 10 matching returns." : string.Empty), PlannerUi.Amber);
    }
    ...
    private void DrawIncomeProjectionSubmarines(IncomeFcProjection fc, IncomeProjectionHorizon horizon)
    {
        ...
        if (!ImGui.BeginTable($"projection-subs-{fc.FcIdKey}", 7, flags,
    ...
    private static void DrawIncomeProjectionSubmarineTooltip(IncomeSubmarineProjection sub, IncomeProjectionHorizon horizon)
    {
        ...
        PlannerUi.WrappedText("Last 90 days · same sector set and exact surveillance, retrieval and favor. Historical sector order is unknown.");
