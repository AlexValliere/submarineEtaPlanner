...
public sealed partial class PlannerWindow
{
    ...
    private void DrawSimulationSettings()
    {
        ...
        using (PlannerUi.SettingRow("ETA model", "Recommended leveling applies an opinionated leveling preset; Custom strategy uses the advanced controls below."))
        {
        ...
        using (PlannerUi.SettingRow("Target rank", $"Choose any supported submarine rank from 1 to {maximumRank}."))
        {
        ...
        using (PlannerUi.SettingRow("Fleet simulation", "Coordinate shared unlocks and voyage timing across submarines in the same free company."))
        {
        ...
        else
        {
            ...
            using (PlannerUi.SettingRow("EXP estimate", "Average is realistic; guaranteed uses the minimum expected reward."))
            {
            ...
            using (PlannerUi.SettingRow("Route scoring", "Prefer EXP per hour instead of maximum EXP per voyage."))
            {
            ...
            using (PlannerUi.SettingRow("Route goal", "Choose which unlock objective the custom strategy should pursue."))
            {
        ...
        using (PlannerUi.SettingRow("Collection delay", "Add a realistic delay between a submarine returning and its next deployment."))
        {
            ImGui.SetNextItemWidth(150f * ImGuiHelpers.GlobalScale);
    ...
    private void DrawRouteSettings()
    {
        ...
        if (settings.EtaModel == EtaModel.PracticalLeveling)
        {
            using (PlannerUi.SettingRow("Maximum voyage duration", "Exclude practical routes longer than this duration. No cap considers every valid route."))
            {
        ...
        else
        {
            ...
            using (PlannerUi.SettingRow("Maximum voyage duration", "Set zero to consider routes of any duration."))
            {
                ImGui.SetNextItemWidth(150f * ImGuiHelpers.GlobalScale);
        ...
        using (PlannerUi.SettingRow("Submarine slots", "Prefer objectives that unlock missing fleet slots before pure leveling routes."))
        {
        ...
        using (PlannerUi.SettingRow("Unknown current voyage", "Choose how the planner handles a deployed submarine whose route cannot be identified."))
        {
    ...
    private void DrawLimitSettings()
    {
        ...
        using (PlannerUi.SettingRow("Per-FC time limit", "Move to the next FC after this many seconds. Set zero for no deadline."))
        {
            ImGui.SetNextItemWidth(150f * ImGuiHelpers.GlobalScale);
        ...
        using (PlannerUi.SettingRow("Voyage safety cap", "Maximum simulated voyages for a single submarine before the result is marked incomplete."))
        {
            ImGui.SetNextItemWidth(150f * ImGuiHelpers.GlobalScale);
    ...
    private void DrawDataSourceSettings()
    {
        ...
        using (PlannerUi.SettingRow("Database override", "Leave empty to use the standard SubmarineTracker location."))
        {
    ...
    private void DrawDisplaySettings()
    {
        ...
        using (PlannerUi.SettingRow("Route diagnostics", "Show per-voyage duration, EXP, and EXP/hour columns in expanded forecasts."))
        {
        ...
        using (PlannerUi.SettingRow("Timeout result", "Keep the last complete forecast or replace it with the newest partial result."))
        {
    ...
    private void DrawSettingsActionBar()
    {
        ...
        if (ImGui.BeginChild("settings-action-bar", new Vector2(-1, -1), true))
        {
            ...
            if (PlannerUi.PrimaryButton("Save changes##apply-settings")) ApplyDraftSettings();
    ...
    private void DrawResetDefaultsModal()
    {
        ...
        ImGui.Spacing();
        PlannerUi.WrappedText("Unapplied edits and custom data-source or route overrides will be replaced in the draft.", PlannerUi.Amber);
        ...
        PlannerUi.SameLineIfFits("Load defaults for review", 30f * ImGuiHelpers.GlobalScale);
    ...
    private static void EndContentPanel()
    {
        ...
        frame.DrawList.AddRectFilled(
            ...
            PlannerTheme.PanelRounding * ImGuiHelpers.GlobalScale);
    ...
    private void BeginSettingsCard(string id, string title, string description)
    {
        BeginContentPanel(id);
        using (this.typography.Heading())
            PlannerUi.WrappedText(title);
