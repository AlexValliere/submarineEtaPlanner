...
public sealed class EtaSimulatorTests
{
    ...
    public void PublicPluginManifestsKeepPositioningAndIdentityInSync()
    {
        ...
        Assert.Equal("1.1.2.0", repoManifest.GetProperty("AssemblyVersion").GetString());
        ...
        Assert.Contains("Income → Projection", repoManifest.GetProperty("Changelog").GetString());
    ...
    public void IncomeUiUsesPlainMetricNames()
    {
        ...
        var fleetHeaders = File.ReadAllText(Path.Combine(repositoryRoot, "src", "SubmarineEtaPlanner", "Ui", "PlannerWindow.FleetHeaders.cs"));
        ...
        Assert.Contains("new ResponsiveTableColumn(\"Avg / day\"", incomePage);
        Assert.Contains("\"Recorded avg/day\"", fleetHeaders);
