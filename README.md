...
Operations and Income fleet headings keep **FC tag** and **World** in separate aligned columns. Income keeps those identity columns sized to their text, with gross gil, recorded average/day, and voyage count spread evenly across the remaining width. Narrow windows wrap the figures below FC tag and World. Expand an FC for the full submarine table and history coverage. Detail columns size automatically to the contents and window, with horizontal scrolling when needed to keep submarine names readable. These remain recorded gross NPC salvage values, with the existing calculation rules.
