...
The plugin reads local SubmarineTracker data, simulates future voyages, and presents ETA plans grouped by FC. Choose the target rank on the Simulation page, apply the change, and the complete dashboard forecast updates to that rank. Recommended leveling applies a ready-to-use preset that follows main sector progression and selects the highest expected-EXP-per-hour route. Active voyages are shown separately from the next route forecast. Custom strategy exposes advanced EXP, route-goal, duration, and build-profile controls. It never deploys submarines, clicks UI, or automates workshop actions.
